...
def save_imgs(out_x, out_y, x, y):
    for i in range(min(len(out_x),10)):
        plt.imsave(f"imgs/result{i}.png",out_x[i].view(img_shape).cpu().numpy())
    return
######################
#  General settings  #
######################
...
#####################
dataset_size = 20000
x_data = torch.Tensor(np.load('trainImages.npy')[:dataset_size])
...
print(x_data.shape)
y_data = torch.Tensor(np.load('trainBoundary.npy')[:dataset_size])
...
ndim_x      = x_train.shape[1] #reduce(lambda x,y: x*y, ndims_x, 1)
ndim_pad_x  = 0
